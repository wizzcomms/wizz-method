...
- `agent_roster` = resolved via `python3 {project-root}/_wizz/scripts/resolve_config.py --project-root {project-root} --key agents` (merges four layers in order: `_wizz/config.toml`, `_wizz/config.user.toml`, `_wizz/custom/config.toml`, `_wizz/custom/config.user.toml`)

...
Load `{project-root}/_wizz/bmm/config.yaml` and resolve `{user_name}`, `{communication_language}`,
...
Present the conclusion, then a concrete next-steps menu: trivial fix → `wizz-quick-dev`; scope/plan adjustment →
`wizz-correct-course`; tracked story → `wizz-create-story`; fresh review → `wizz-code-review`. Recommend the

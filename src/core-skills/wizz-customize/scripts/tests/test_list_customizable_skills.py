...
- a pre-existing team override in _wizz/custom/
...
class ScannerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.custom = self.root / "_wizz" / "custom"
    ...
    def test_agent_only_skill_detected(self):
        ...
        self.assertEqual(entry["name"], "wizz-agent-pm")
    ...
    def test_existing_team_override_flagged(self):
        ...
        (self.custom / "wizz-agent-pm.toml").write_text("[agent]\n")
        result = MODULE.scan_skills([self.skills], self.root)
        entry = result["agents"][0]
    ...
    def test_multiple_skills_roots_are_merged(self):
        ...
        self.assertEqual(names, {"wizz-agent-pm", "wizz-agent-dev"})

...
description: >
  ...
  /iniciar, /salvar, /ver, /dia, /dump, /decisao, /conteudo, /prospect ou /memoria. Também ative quando o
  usuário mencionar "cerebro", "atualizar o cerebro", "salvar sessão", "resumo do projeto",
  "estado atual", "registrar decisão", "ideia de conteúdo", "pesquisar prospect" ou "ler a auto-memória".
  ...
  A auto-memória do agente é a outra camada, e `/memoria` é a ponte de leitura dela para
  qualquer plataforma (Codex, OpenCode, Gemini) rodando na mesma máquina.
  Use quando o usuário digitar um desses comandos de barra ou mencionar memória, sessão,
  decisão, ideia de conteúdo, prospect ou auto-memória.
---
...
**Regra de links:** sempre wiki-links `[[arquivo]]` dentro do vault.
...
- `references/comandos-pesquisa-e-memoria.md` — `/prospect` (pesquisar prospect) e `/memoria` (ler e alimentar a auto-memória do projeto de qualquer plataforma) com passos e templates completos.

## Regras gerais

- Wiki-links `[[arquivo]]` dentro do vault.
